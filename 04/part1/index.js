let line1 = "aa bb cc";
let line2 = "bb cc dd cc ee";
let line1Bits = line1.split(' ');
console.log(line1Bits);
let line2Bits = line2.split(' ');
console.log(line2Bits);

let foundDupe = false;
line1Bits.forEach(element => {
    let matching = line1Bits.filter(e => e == element);
    if (matching.length > 1) {
        foundDupe = true;
    }
});
if (foundDupe) console.log('found dupe in line1');

foundDupe = false;
line2Bits.forEach(element => {
    let matching = line2Bits.filter(e => e == element);
    if (matching.length > 1) {
        foundDupe = true;
    }
});
if (foundDupe) console.log('found dupe in line2');

function hasDupes(line) {
    foundDupe = false;
    let lineBits = line.split(' ');
    lineBits.forEach(element => {
        let matching = lineBits.filter(e => e == element);
        if (matching.length > 1) {
            foundDupe = true;
        }
    });
...
console.log(`non dupe count: ${nonDupeCount}`);