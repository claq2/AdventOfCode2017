...
function compute(row) {
    let delim = row.indexOf('\t') !== -1 ? '\t' : ' ';
    let values = row.split(delim).map(Number);
    values = values.sort().reverse();
    console.info('values: ', values);
    for (let i = 0; i < values.length; i++) {
        const ielement = values[i];
        for (let j = 0; j < values.length; j++) {
            const jelement = values[j];
            let mod = ielement / jelement;
            console.log(`${ielement} / ${jelement} is ${mod}`);
            if (mod === Math.round(mod) && mod !== 1) {
                return ielement / jelement;
            }
        }
    }
}
...
tests.rows.forEach((row) => {
    let computed = compute(row.input);
    total += computed;
    if (computed !== row.output) {
        console.error(`Expected ${row.output} for ${row.input} but got ${computed}`);
        errors++;
    }
});

if (total !== tests.total) {
    console.error(`Expected total ${tests.total} but got ${total}`);
    errors++;
}
...
if (errors === 0) {
    ...
    let total = 0;
    input.forEach((row) => {
        let computed = compute(row);
        total += computed;
    });
    console.info(`The solution is ${total}`);
} else {
